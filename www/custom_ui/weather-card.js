class WeatherCard extends HTMLElement {
  set hass(hass) {
    ...
    const getUnit = function (measure) {
      const lengthUnit = km;//hass.config.unit_system.length;
      switch (measure) {
        case 'air_pressure':
    ...
    this.content.innerHTML = `
      ...
      <span class="temp">${temperature}</span><span class="tempc"> °C</span>
      <span>
        <ul class="variations right">
            <li><span class="ha-icon"><ha-icon icon="mdi:water-percent"></ha-icon></span>${humidity}<span class="unit"> %</span></li>
            <li><span class="ha-icon"><ha-icon icon="mdi:gauge"></ha-icon></span>${pressure}<span class="unit"> hPa</span></li>
        </ul>
        <ul class="variations">
            <li><span class="ha-icon"><ha-icon icon="mdi:weather-windy"></ha-icon></span>${windBearing} ${windSpeed}<span class="unit"> m/s</span></li>
            <li><span class="ha-icon"><ha-icon icon="mdi:weather-fog"></ha-icon></span>${visibility}<span class="unit"> km</span></li>
        </ul>
      </span>
      <div class="forecast clear">
          ${forecast.map(daily => `
              <div class="day">
                  ...
                  <br><span class="highTemp">${daily.temperature}'°C'</span>
                  <br><span class="lowTemp">${daily.templow}'°C'</span>
              </div>`).join('')}
      </div>`;
  }
